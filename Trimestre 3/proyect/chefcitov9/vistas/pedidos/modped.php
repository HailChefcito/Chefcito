<?php
  ...
  $idPed=$_GET['idPedido'];
  $queryMostrar2 = mysqli_query($conn, "SELECT emailUsuario FROM usuarios");
  $querypedi = mysqli_query($conn, "SELECT idPedidos, emailUsuario, fechaPedido, horaPedido, valorPedido FROM pedidos WHERE idPedidos='$idPed'");

    while($mostrar = mysqli_fetch_array($querypedi))
    {
        $numtel = $mostrar['emailUsuario'];
 ...
 if(isset($_POST['btnmodif']))
    {
        ...
        $querymodificar = mysqli_query($conn,"UPDATE pedidos SET emailUsuario='$numtel1',fechaPedido='$fecped1',horaPedido='$horaped1',valorPedido='$valorped1' WHERE idPedidos=$idPed");
